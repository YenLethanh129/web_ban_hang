        ...
        : FrmBaseManagement<EmployeeManagementModel, EmployeeManagementPresenter>
    {
        ...
        private async void OpenEmployeeDetailsDialog(EmployeeViewModel? selectedEmployee = null)
        {
            try
            {
                ...
                var detailsPresenter = new EmployeeDetailsPresenter(
                    _serviceProvider.GetRequiredService<IEmployeeManagementService>(),
                    _serviceProvider.GetRequiredService<IPayrollService>(),
                    _serviceProvider.GetRequiredService<IEmployeeShiftService>(),
                    _serviceProvider.GetRequiredService<IBranchService>(),
                    _serviceProvider.GetRequiredService<IRoleManagementService>(),
                ...
                EmployeeDetailViewModel? initialModel = null;

                if (selectedEmployee != null)
                {
                    initialModel = new EmployeeDetailViewModel
                    {
                ...
                var detailForm = new FrmEmployeeDetails(detailsPresenter, selectedEmployee!.Id, initialModel);

                var result = detailForm.ShowDialog(this);

                if (result == DialogResult.OK)
                {
                    var updatedEmployee = detailForm.Employee;

                    if (selectedEmployee != null)
                    {
                        await HandleEmployeeUpdate(updatedEmployee);
                        ShowInfo("Cập nhật thông tin nhân viên thành công!");
                    }
                    else
                    {
                        await HandleEmployeeAdd(updatedEmployee);
                        ShowInfo("Thêm nhân viên mới thành công!");
                    }
            ...
            {
                ShowError($"Lỗi khi {(selectedEmployee != null ? "cập nhật" : "thêm")} nhân viên: {ex.Message}");
            }
            finally
            {
                SetLoadingState(false);
